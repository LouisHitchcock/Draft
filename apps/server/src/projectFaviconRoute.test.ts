...
describe("tryHandleProjectFaviconRequest", () => {
  ...
  it("serves a well-known favicon file from the project root", async () => {
    const projectDir = makeTempDir("draft-favicon-route-root-");
  ...
  it("resolves icon href from source files when no well-known favicon exists", async () => {
    const projectDir = makeTempDir("draft-favicon-route-source-");
  ...
  it("resolves icon link when href appears before rel in HTML", async () => {
    const projectDir = makeTempDir("draft-favicon-route-html-order-");
    const iconPath = path.join(projectDir, "public", "brand", "logo.svg");
    fs.mkdirSync(path.dirname(iconPath), { recursive: true });
    fs.writeFileSync(
      path.join(projectDir, "index.html"),
  ...
  it("resolves object-style icon metadata when href appears before rel", async () => {
    const projectDir = makeTempDir("draft-favicon-route-obj-order-");
  ...
  it("serves a fallback favicon when no icon exists", async () => {
    const projectDir = makeTempDir("draft-favicon-route-fallback-");
  ...
  it("does not follow symlinked well-known favicon files outside the project root", async () => {
    ...
    const projectDir = makeTempDir("draft-favicon-route-symlink-root-");
    const externalDir = makeTempDir("draft-favicon-route-external-root-");
  ...
  it("does not follow symlinked icon href targets outside the project root", async () => {
    ...
    const projectDir = makeTempDir("draft-favicon-route-symlink-href-");
    const externalDir = makeTempDir("draft-favicon-route-external-href-");
    const externalIconPath = path.join(externalDir, "outside.svg");
