...
export const PI_PROVIDER_SETUP_MESSAGE =
  "Draft embeds Pi through the Pi Node SDK. Authenticate Pi outside Draft through the Pi CLI (`pi` or `bunx pi`) and `/login`, or populate ~/.pi/agent/auth.json / provider env vars. Draft intentionally disables Pi packages, extensions, prompt templates, skills, themes, AGENTS, and custom system-prompt discovery so Draft remains the only source of workspace instructions here.";

export const PI_PLAN_MODE_PROMPT_PREFIX = `<collaboration_mode name="plan">
You are in Draft plan mode.
...
- When you present the finalized plan, wrap it in <proposed_plan>...</proposed_plan> so Draft can render it specially.
</collaboration_mode>`;
