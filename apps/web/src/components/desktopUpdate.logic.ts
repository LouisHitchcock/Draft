import type { DesktopUpdateActionResult, DesktopUpdateState } from "@draft/contracts";
...
export function getArm64IntelBuildWarningDescription(
  ...
  if (action === "download") {
    return language === "fa"
      ? "این مک Apple Silicon دارد، اما Draft هنوز نسخه اینتل را با Rosetta اجرا می کند. برای رفتن به نسخه بومی Apple Silicon، به روزرسانی موجود را دانلود کنید."
      : "This Mac has Apple Silicon, but Draft is still running the Intel build under Rosetta. Download the available update to switch to the native Apple Silicon build.";
  }
  if (action === "install") {
    return language === "fa"
      ? "این مک Apple Silicon دارد، اما Draft هنوز نسخه اینتل را با Rosetta اجرا می کند. برای نصب نسخه دانلودشده Apple Silicon دوباره راه اندازی کنید."
      : "This Mac has Apple Silicon, but Draft is still running the Intel build under Rosetta. Restart to install the downloaded Apple Silicon build.";
  }
  return language === "fa"
    ? "این مک Apple Silicon دارد، اما Draft هنوز نسخه اینتل را با Rosetta اجرا می کند. به روزرسانی بعدی برنامه آن را با نسخه بومی Apple Silicon جایگزین می کند."
    : "This Mac has Apple Silicon, but Draft is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
