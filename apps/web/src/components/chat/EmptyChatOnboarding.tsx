...
function getEmptyChatOnboardingCopy(language: "en" | "fa") {
  if (language === "fa") {
    return {
      ...
      loadingDescription: "Draft در حال همگام سازی فضای کاری محلی شما است.",
      noProjectsTitle: "اولین پروژه خود را اضافه کنید",
      noProjectsDescription:
        "Draft به یک پوشه پروژه نیاز دارد تا بتواند thread ها را ایجاد کند، دستورهای محلی را کشف کند، و AGENTS.md و skill های فضای کاری را بخواند.",
      browse: "مرور پوشه",
      addProject: "افزودن پروژه",
      pathLabel: "مسیر پروژه",
      pathPlaceholder: "/path/to/project",
      nextStepHint: "بعد از افزودن پروژه، Draft فوراً اولین thread پیش نویس را برای شما باز می کند.",
      existingProjectsTitle: "یک thread جدید شروع کنید",
      existingProjectsDescription: (count: number) =>
        `${count} پروژه از قبل در Draft موجود است. می توانید یک thread جدید بسازید یا از سایدبار یک thread قبلی را ادامه دهید.`,
  ...
  return {
    ...
    loadingDescription: "Draft is syncing the local workspace state.",
    noProjectsTitle: "Add your first project",
    noProjectsDescription:
      "Draft needs a project folder before it can create threads, discover repo-local commands, and read workspace AGENTS.md and skills.",
    browse: "Browse for folder",
    addProject: "Add project",
    pathLabel: "Project path",
    pathPlaceholder: "/path/to/project",
    nextStepHint:
      "After you add a project, Draft immediately opens your first draft thread so you can start working right away.",
    existingProjectsTitle: "Start a new thread",
    existingProjectsDescription: (count: number) =>
      `${count} project${count === 1 ? " is" : "s are"} already available in Draft. Start a new thread or resume one from the sidebar.`,
