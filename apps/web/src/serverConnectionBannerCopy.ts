...
export function getServerConnectionBannerDescription(args: {
  ...
  if (args.language === "fa") {
    if (args.retrying) {
      return args.isElectron
        ? "برنامه به صورت خودکار دوباره اتصال وب سوکت را امتحان می کند. اگر این وضعیت ادامه داشت، Draft را دوباره راه اندازی کنید."
  ...
  if (args.retrying) {
    return args.isElectron
      ? "The app is retrying the websocket connection automatically. If this keeps happening, restart Draft."
