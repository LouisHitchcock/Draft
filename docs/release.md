...
This document covers how Draft desktop releases are built, optionally signed, and published from one tag.
...
- Builds four desktop artifacts in parallel:
  - macOS `arm64` DMG and ZIP, named `Draft-macOS-<version>-<arch>.<ext>`
  - macOS `x64` DMG and ZIP, named `Draft-macOS-<version>-<arch>.<ext>`
  - Linux `x64` AppImage, named `Draft-linux-<version>-<arch>.<ext>`
  - Windows `x64` NSIS installer, named `Draft-windows-<version>-<arch>.<ext>`
- Publishes one GitHub Release with all produced files.
  - Versions with a suffix after `X.Y.Z` (for example `1.2.3-alpha.1`) are published as GitHub prereleases.
  - Only plain `X.Y.Z` releases are marked as the repository's latest release.
  - Desktop prerelease artifacts launch as `Draft`, the same as stable builds.
  - The GitHub Release title is `Draft v<version>`.
...
- Optionally publishes the CLI package (`apps/server`, npm package `draft`) when explicitly enabled.
...
- Repository slug source:
  - `DRAFT_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `DRAFT_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
- repository variable `DRAFT_PUBLISH_CLI=true` for tag-triggered releases.
...
1. Confirm npm org/user owns package `draft`.
...
- Tag-triggered releases:
  - set repository variable `DRAFT_REQUIRE_SIGNING=true`
- Manual releases:
...
9. If you want stable releases to fail when signing is unavailable, enable `DRAFT_REQUIRE_SIGNING=true`.
...
8. If you want stable releases to fail when signing is unavailable, enable `DRAFT_REQUIRE_SIGNING=true`.
...
3. Decide whether signing must be enforced for this run:
   - repository variable `DRAFT_REQUIRE_SIGNING=true`, or
...
- macOS build is unsigned when it should be signed:
  ...
  - enable `DRAFT_REQUIRE_SIGNING=true` so the workflow fails instead of silently continuing unsigned
