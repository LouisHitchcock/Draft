...
describe("create-release-checksums", () => {
  it("writes stable SHA256SUMS entries for release assets", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "draft-release-checksums-"));

    try {
      writeFileSync(resolve(rootDir, "Draft-linux-1.0.0-x86_64.AppImage"), "linux-app");
      writeFileSync(resolve(rootDir, "latest.yml"), "channel-manifest");
      mkdirSync(resolve(rootDir, "nested"), { recursive: true });
      writeFileSync(resolve(rootDir, "nested", "Draft-macOS-1.0.0-arm64.zip"), "mac-zip");
      writeFileSync(resolve(rootDir, "SHA256SUMS"), "stale-manifest");

      const entries = createReleaseChecksums(rootDir);
      assert.deepStrictEqual(
        entries.map((entry) => entry.path),
        ["Draft-linux-1.0.0-x86_64.AppImage", "latest.yml", "nested/Draft-macOS-1.0.0-arm64.zip"],
      );

      const serialized = serializeReleaseChecksums(entries);
      assert.equal(serialized.split("\n").length, 3);
      assert.ok(serialized.includes("Draft-linux-1.0.0-x86_64.AppImage"));
      assert.ok(serialized.includes("nested/Draft-macOS-1.0.0-arm64.zip"));
